...
class Scheduler:
    ...
    def run(self) -> None:
        ...
        logger.info("scheduler.start", "Scheduler starting", 
        ...
        try:
            ...
            logger.info("scheduler.ready", "Scheduler ready, entering main loop",
            ...
            logger.error("scheduler.error", f"Scheduler error: {e}",
    ...
    def _run_main_loop(self) -> None:
        ...
        while not self._shutdown:
            ...
            with start_span("scheduler.loop_iteration", {"iteration": iteration_count}):
                ...
                runnable_count = len([state for state in self._plan.ready_states.values() 
                ...
                priority_label = {"band": priority.name}
                ...
                if ready_state.message_ready:
                    with with_context(node=node_name):
                        logger.debug("scheduler.process_messages", 
                ...
                # Process tick if no messages or if still tick-ready
                    ...
                    with with_context(node=node_name):
                        logger.debug("scheduler.process_tick", 
                                   f"Processing tick for node {node_name}")
    ...
    def set_capacity(self, edge_id: str, capacity: int) -> None:
        ...
        if self._running:
            ...
            if edge_id in self._plan.edges:
                ...
                logger.info("scheduler.capacity_changed", 
            ...
            logger.warn("scheduler.capacity_not_supported", 
                       "Capacity changes not supported before runtime")
        ...
        for edge in edges:
            ...
            with with_context(node=node.name, port=port, edge_id=edge._edge_id()):
                if result == PutResult.BLOCKED:
                    logger.debug("scheduler.backpressure", 
                               "Message blocked, applying backpressure")
                    # TODO: Implement cooperative yielding for backpressure
                elif result == PutResult.DROPPED:
                    logger.warn("scheduler.message_dropped", 
                              "Message dropped due to capacity limits")
                else:
                    logger.debug("scheduler.message_routed", 
