...
from arachne.observability.metrics import get_metrics, PrometheusMetrics
from arachne.observability.tracing import get_tracer, InMemoryTracer
...
def test_observability_integration() -> None:
    ...
    print(f"   - Recorded {len(all_counters)} counters, {len(all_gauges)} gauges, {len(all_histograms)} histograms")
