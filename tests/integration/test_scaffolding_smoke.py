...
class TestScaffoldingSmoke:
    ...
    def test_node_generator_cli(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([
                sys.executable, "-m", "arachne.scaffolding.generate_node",
                "--name", "SmokeTestNode",
                "--package", "smoke_test",
    ...
    def test_subgraph_generator_cli(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([
                sys.executable, "-m", "arachne.scaffolding.generate_subgraph",
                "--name", "SmokeTestPipeline",
                "--package", "smoke_test",
                "--dir", temp_dir,
                "--include-tests",
                "--force"
    ...
    def test_cli_error_handling(self):
        ...
        result = subprocess.run([
            sys.executable, "-m", "arachne.scaffolding.generate_node",
            "--name", "invalid-name",  # Invalid: contains dash
            "--package", "test"
        ], capture_output=True, text=True, cwd=os.getcwd())
    ...
    def test_complex_port_types(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run([
                ...
                "--package", "complex",
                "--inputs", "data:List[Dict[str, Any]],config:Optional[str]",
                "--outputs", "results:Dict[str, List[int]]",
                "--dir", temp_dir,
                "--force"
            ], capture_output=True, text=True, cwd=os.getcwd())
    ...
    def test_generated_code_line_count(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run([
                ...
                "--name", "ComplexNode",
                "--package", "complex",
                "--inputs", "in1:dict,in2:str,in3:int",
                "--outputs", "out1:dict,out2:str,out3:int",
                "--dir", temp_dir,
                "--force"
            ], check=True, capture_output=True, cwd=os.getcwd())
            ...
            # Should be well under 200 lines
            assert line_count < 200, f"Generated subgraph has {line_count} lines, should be under 200" 