...
def test_priority_scheduling() -> None:
    ...
    data_edge_id = sg.connect(("DataProducer", "out"), ("DataConsumer", "in"), capacity=1)
...
def test_fairness_ratio() -> None:
    ...
    normal_edge = sg.connect(("NormalPriorityProducer", "out"), ("FairnessConsumer", "in"), capacity=10)
    
    config = SchedulerConfig(
        tick_interval_ms=5,
        fairness_ratio=(4, 2, 1),  # control:high:normal
...
def test_registration_validation() -> None:
    ...
    try:
        sch.run()
        assert False, "Should raise ValueError for duplicate node names"
