...
class TestValidatePorts:
    ...
    def create_mock_node(self, inputs_dict=None, outputs_dict=None, inputs_callable=True, outputs_callable=True):
    ...
    def test_valid_node_ports(self):
        ...
        node = self.create_mock_node(
            inputs_dict={"input1": Mock(), "input2": Mock()},
            outputs_dict={"output1": Mock()}
        )
...
class TestValidateGraph:
    ...
    def create_mock_subgraph(self, nodes=None, edges=None, exposed_inputs=None, exposed_outputs=None):
    ...
    def test_valid_subgraph(self):
        ...
        subgraph = self.create_mock_subgraph(
            nodes=[node1, node2],
            exposed_inputs=["input1"],
            exposed_outputs=["output1"]
        )
...
class TestPydanticAdapter:
    ...
    @pytest.mark.skipif(True, reason="Pydantic integration test - would require pydantic dependency")
